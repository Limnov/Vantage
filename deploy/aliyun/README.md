...
4. 只传输 `server/src`、`server/scripts/seed-demo.js`、`server/scripts/create-trial-account.js`、server 包清单、`db`、根包清单和 `web/dist`；在 Linux 上运行 `PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=1 npm ci --omit=dev --prefix server`。不要上传本地数据库、日志或 macOS 的 node_modules。切换 `current` 前，把上一版本的内容哈希静态文件合并进新版本：`cp -an /opt/vantage/current/web/dist/assets/. <new-release>/web/dist/assets/`。这能保证 Cloudflare 短时缓存的旧 HTML 在过渡期仍能加载对应资源。
...
7. 验证源站后，将 Cloudflare 的 `vantage` 记录设置为服务器 A 记录并开启代理。旧 Worker 自定义域名必须先解除；不要同时保留两个入口。Cloudflare 到源站应使用 Full 或 Full (strict)。Cache Rule “Vantage 页面短时边缘缓存”只覆盖公开 HTML 路由，Edge TTL 使用 Free 方案允许的最短 2 小时，浏览器继续接受源站的 `no-cache`；`/api`、`/mcp`、`/health` 不得加入规则。每次发布后清除这些 HTML URL 的 Cloudflare 缓存，让新版本立即生效。
